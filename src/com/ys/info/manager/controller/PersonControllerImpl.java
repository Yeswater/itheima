...
import com.ys.info.manager.service.PersonService;
import com.ys.info.manager.service.PersonServiceImpl;
...
public class PersonControllerImpl implements PersonController {
    ...
    public void add() {
        ...
        while (true) {
            ...
            id = sc.nextLine();
            boolean flag = service.isExist(id,ps);
            if (flag) {
    ...
    public void delete() {
        ...
        if (service.isExist(id,ps)) {//判断学生/老师是否存在
            service.delete(id,ps);
    ...
    public void query() {
        ...
        switch (sc.nextLine()) {
            case "1":
                System.out.print("-> 学号：");
                String id = sc.nextLine();
                if (service.isExist(id,ps)) {
                    Person person = service.query(id,ps);//接收service层的结果并自定义输出
                    MyPrint.firstLine();
                ...
                break;
            case "2":
                Person[] arr = service.queryAll(ps);//接收service层的结果并自定义输出
                System.out.println();
                MyPrint.firstLine();
                MyPrint.table("ID", "name", "age");
